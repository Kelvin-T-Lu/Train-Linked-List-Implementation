...
class Simulator {
	...
	/**
	 * Sets up the intital train position based on scanner. 
	 * @param trainScanner The scanner where it's being read from.
	 */
	public void setupInitialTrainPositions(Scanner trainScanner) {
		...
		while(trainScanner.hasNextLine()) {
			...
			if(s.length()==0) {
				return; //if the line is blank, we're done reading the train's initial positions
			}
